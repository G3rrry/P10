...
<body>
    ...
    <div class="container mt-5">
        ...
        <p>Aquí se muestran los libros registrados en la base de datos.</p>
        ...
        <div class="row row-cols-1 row-cols-md-3 g-4 mt-3">
            <?php if ($result->num_rows > 0): ?>
                <?php while($row = $result->fetch_assoc()): ?>
                    <div class="col">
                        <div class="card h-100">
                            <img src="data:image/jpeg;base64,<?= base64_encode($row['imagen_portada']); ?>" class="card-img-top" alt="Portada del Libro">
                            <div class="card-body">
                                <h5 class="card-title"><?= htmlspecialchars($row['titulo']); ?></h5>
                                <p class="card-text">Autor: <?= htmlspecialchars($row['autor']); ?></p>
                            </div>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <p class="text-center mt-4">No hay libros registrados aún.</p>
            <?php endif; ?>
        </div>
    </div>
